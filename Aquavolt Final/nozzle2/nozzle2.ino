...
void loop() {
  ...
  if (MySerial.available()) {
        String serialInput = MySerial.readStringUntil('\n');
        serialInput.trim();
        Serial.println("Received from MySerial: " + serialInput);

      
        // Update relay state based on "Approved" status and SOC
        if (serialInput == "Approved" && (soc < 100)) {
            digitalWrite(RELAY_PIN, LOW);  // Activate (open) the relay for fueling
            Serial.println("Relay Opened - Fueling");
        } else {
            digitalWrite(RELAY_PIN, HIGH);  // Deactivate (close) the relay
